...
**Offline use:** the public Cloudflare site requires an internet connection. For private offline chat, install Ollama and download a model on that device, then run this Python app locally and open `http://127.0.0.1:8000`. The cached website shell alone cannot run the AI model offline. Cloud image generation and other hosted services also require internet.
...
This is the first backend release. Text chat, guest history, image generation, and summaries for PDF, TXT, Markdown, CSV, and DOCX files are implemented. Uploaded documents are sent to Cloudflare for conversion and summarization; their contents are not saved in chat history. Image generation is capped at two per guest per day and 30 per day site-wide to help stay within shared free usage. Email signup/password reset, image questions, PowerPoint summaries, and voice transcription are not implemented in the Worker yet. The local Python/Ollama app continues to provide its existing local features. Cloudflare Free has usage caps: Workers allow 100,000 requests/day; D1 includes 500 MB per database with daily read/write limits; Workers AI includes a shared 10,000-neuron daily allowance. Requests can stop until limits reset. See [Workers limits](https://developers.cloudflare.com/workers/platform/limits/), [D1 pricing and limits](https://developers.cloudflare.com/d1/platform/pricing/), and [Workers AI pricing](https://developers.cloudflare.com/workers-ai/platform/pricing/).
